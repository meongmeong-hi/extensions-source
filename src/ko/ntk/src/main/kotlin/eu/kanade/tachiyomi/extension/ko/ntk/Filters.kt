...
// ================= 소설(Novel) 필터 (스크린샷 반영 완료) =================
class NvSortFilter : Filter.Select<String>("정렬", sortList.map { it.name }.toTypedArray())
class NvStatusFilter : Filter.Select<String>("상태", statusList.map { it.name }.toTypedArray())
class NvGenreFilter : Filter.Group<GenreTriState>("장르", nvGenreList.map { GenreTriState(it) })

// 스크린샷 주소(sbxh9.com) 기준 실제 활성화된 장르 목록 매핑
...
fun buildNvGenreParam(genreFilter: NvGenreFilter?): String? {
    ...
    val genres = genreFilter.state
        ...
        .mapIndexedNotNull { index, triState ->
            when (triState.state) {
                Filter.TriState.STATE_INCLUDE -> nvGenreList[index]
                Filter.TriState.STATE_EXCLUDE -> "-${nvGenreList[index]}"
                else -> null
            }
        }
    return if (genres.isNotEmpty()) genres.joinToString(",") else null
